...
The requirements this skill implements are in `docs/superpowers/specs/2026-08-27-review-pr-design.md` (rule IDs R1–R7); this file tells the calling session how to run it and does not restate those rules.
...
4. Relay, reading `child_json_path`, `stderr_path`, `exit`, and `kept` from `<scratchpad>/review-pr-child.json`; the two paths are readable whether or not the scratch directory was kept.
...
- If `exit` is non-zero, or `child.json` is missing or not valid JSON, or `.is_error` is true: say "The review did not complete." and show the last 30 lines of `child.stderr`.
...
- Finish with one line: `cost $<total_cost_usd> · <duration_ms as minutes> min · exit <exit> · scratch removed` (or `scratch kept <dir>` when `REVIEW_PR_KEEP=1`).
