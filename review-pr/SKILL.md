...
- Per R5.5: `(none)` in `.result` is a clean review, not a skipped one. Add "The PR was not reviewed; an empty findings list here is not a clean result." only when `.result` states the diff could not be obtained. A non-empty `.permission_denials` does not by itself mean the PR was not reviewed (the lock-down denies tools like `uv` and `pytest` by design); when it is non-empty, add one informational line after the fenced block: "N tool calls were denied under the lock-down." (N is the length of `.permission_denials`).
