...
- Per R5.5: `(none)` in `.result` is a clean review, not a skipped one. Add "The PR was not reviewed; an empty findings list here is not a clean result." only when `.permission_denials` in the file at `child_json_path` is non-empty, or `.result` states the diff could not be obtained.
