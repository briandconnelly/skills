...
`_meta` namespacing is itself a surface concern: it is MCP-specific, so keep the semantic error codes and metadata identical across surfaces but let each carry them in its own envelope — `_meta` or `error.data` on MCP, exit codes plus stderr on a CLI, language-native exceptions on a code API.
