...
- Note optional MCP capabilities each task benefits from or requires: roots, completions, resource subscriptions, elicitation, list-change notifications, and tasks. Record the weaker-client fallback at the same time.
...
- Implement `completion/complete` for prompt arguments and resource-template variables with dynamic value sets; document that completion does not cover arbitrary tool arguments.
