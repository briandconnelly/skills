...
- **Negotiated capability**: an optional MCP feature the client declared in its per-request `clientCapabilities` or the server advertised via `server/discover`; agents must not assume it exists from schema prose alone.
- **Root**: a client-declared filesystem boundary, obtainable via MRTR (`ListRootsRequest`) from clients that still declare the deprecated `roots` capability; workspace scope is primarily ordinary tool arguments, and roots are guidance rather than access control.
...
- **Task-returning tool**: a tool that may answer with a task (`resultType: "task"`) under the negotiated `io.modelcontextprotocol/tasks` extension (contract-checklist §7 owns the gating rules), so clients can recover status and results after the original call returns.
