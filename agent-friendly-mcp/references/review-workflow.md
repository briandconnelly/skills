...
- **Capability gating.** Which optional MCP capabilities does the server rely on after initialization? Verify that roots, completions, resource subscriptions, elicitation, tasks, and list-change notifications are advertised before use, and that weaker clients get a structured fallback instead of a mysterious method failure. *(maps to §1, §2, §4, §6, §7, §9)*
- **Resource freshness.** If the agent holds a resource URI across turns, can it tell whether the catalog changed vs. the resource body changed? Check `listChanged`, `resources/subscribe`, `notifications/resources/updated`, and `annotations.lastModified` behavior where available. *(maps to §4, §9)*
