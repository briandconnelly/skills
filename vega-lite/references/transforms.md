...
- **Filter runs before encoding-level `aggregate`, not after.** A `filter` transform only ever sees raw row values, so `{"filter": "datum.count > 5"}` fails if `count` is a field produced by an encoding's `"aggregate": "count"` rather than a raw column, since the aggregate hasn't happened yet when the transform array runs — to filter on an aggregated result, aggregate the rows first and filter that output instead.
Do that either upstream in the data pipeline, or with a view-level `aggregate` entry in the `transform` array placed before the `filter` — `{"aggregate": [{"op": "count", "as": "n"}], "groupby": ["category"]}`, using the same ops as the encoding-level `aggregate` in `references/authoring-basics.md` — so that the later `{"filter": "datum.n > 5"}` sees a real `n` column.
(The `transform`-array `aggregate` is shown here only to unblock this filter-after-aggregate case; v1 otherwise teaches aggregation at the encoding level.)
