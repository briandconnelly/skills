...
2. *Control-plane tampering (T10).* A solo maintainer holds repository admin, so the shared-credential agent holds it too — and a repository admin can edit or delete the very ruleset these gates live in.
   ...
   Because the boundary cannot live in repository configuration here, it moves to the harness: the agent's permission configuration MUST deny the mutating administrative surface — rulesets and branch protection, repository and Actions settings, environments and secrets, App installations, and merge — via both `gh`/`git` command rules and the equivalent REST paths.
   Where the repository is organization-owned, prefer an **organization-level** ruleset targeting it, which a repository admin cannot edit; that restores an enforced boundary and reduces this gap to merge authority alone.
...
- For an organization-owned repository, the protected-branch ruleset is created at the ORGANIZATION level targeting this repo, not at the repository level.
  A repository admin can edit or delete a repository-level ruleset but not an organization-level one, so this is the only placement that survives an over-permissioned repo-scoped identity.
  Repository-level is acceptable for a user-owned repo, where no higher scope exists — there the §4 no-admin precondition is doing all the work. (closes T10)
...
- Tag and release refs are protected, not just branches: a tag ruleset (`"target": "tag"`) on the release tag pattern blocks creation, deletion, and force-update by any non-bypass actor, and the agent identity holds no release or package write permission by default.
  A branch-only ruleset leaves an agent with `contents: write` free to create or move a release tag and publish a release from an unmerged commit — no protected branch changes, and consumers execute the artifact anyway.
  Where releases are automated, the publishing workflow runs from a protected ref behind an environment gate (see the production environment gate in examples/rulesets.md) rather than from PR context. (closes T11)
...
- No `[bot]` approval counts toward the required review threshold: an App with Pull requests write can submit approving reviews, and GitHub is understood to count them like any write-access reviewer's — and the agent's App needs that permission to open PRs, so the grant cannot be dropped.
  ...
  There is no setting that excludes bots from counting, so enforce it as the human-only-approvals required status check (worked example in examples/required-checks.md): triggered on `pull_request` and `pull_request_review`, failing while any live approving review on the PR comes from a `[bot]` actor.
...
- OIDC is used for cloud authentication instead of stored long-lived secrets, AND the cloud-side trust policy is verified — not merely the workflow side.
  ...
  The cloud role's trust condition must pin `aud` and constrain `sub` to this repository AND to a specific ref or environment (`repo:<org>/<repo>:environment:production`, or `repo:<org>/<repo>:ref:refs/heads/main`), never a bare `repo:<org>/<repo>:*` that any branch or fork-triggered workflow can satisfy; prefer `job_workflow_ref` where the provider supports it, which pins the exact workflow file.
...
- Any workflow that opens PRs or pushes branches on the agent's behalf authenticates as the agent's App (e.g. `actions/create-github-app-token`), NOT with the default `GITHUB_TOKEN`.
  Events raised by `GITHUB_TOKEN` do not trigger further workflow runs, so a PR opened with it never fires the `pull_request` workflows behind the required checks — the checks sit PENDING forever and the PR can never merge.
  The failure is fail-safe but silent, and it looks exactly like flaky CI, which §2 warns is how bypass habits start. (productivity; closes T4 indirectly)
