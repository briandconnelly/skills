...
- Linear history is required, and `allowed_merge_methods` is restricted to `squash` and `rebase` to match — leaving the `merge` method enabled alongside required linear history is contradictory (a merge commit cannot satisfy linear history, so the option is a dead end that only misleads). (closes T8)
