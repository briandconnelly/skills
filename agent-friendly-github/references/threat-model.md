...
The same Pull requests write permission that lets the App open PRs also lets it submit approving reviews, which GitHub counts like any write-access reviewer's; close that side with the human-only-approvals required check (§2, worked example in examples/required-checks.md).
