...
A non-empty bypass list is a finding only when it contains an automation identity the agent can act as; a human-maintainer bypass in the solo profile is expected, not a defect (small-team and org repos keep the list empty — a second human reviewer unblocks merges without a bypass).
...
- **Bypass-actors list** — retrieve the default-branch ruleset and confirm `bypass_actors` contains no automation identity the agent can act as (its GitHub App / `Integration`, a bot PAT, a deploy key, or a role such as `Write` that the agent holds). A human-maintainer entry (an individual `User`, or `Maintain`/`Repository admin` the agent cannot hold) is expected in the solo profile and is not a finding; an empty list is expected in small-team and org/high-risk repos (a second human reviewer unblocks merges without a bypass).
...
- **Remediation:** Remove every automation identity from `bypass_actors` — the agent App and any role the agent can hold (config-checklist.md §2: "no automation identity ... may appear in the bypass-actors list"). In the solo profile a human-maintainer `User` entry with `bypass_mode: pull_request` may remain as the documented escape hatch; in small-team and org/high-risk repos the list should be empty.
