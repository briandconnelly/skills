...
- Merge authority defaults to the human — an agent does not merge a PR it authored unless merge authority was explicitly delegated (a standing grant in `AGENTS.md` or an in-session instruction that names merging); green required checks and a zero-review configuration are gate outcomes, not delegation.
