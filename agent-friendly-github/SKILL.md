...
- **Bypass-actors list**: the set of identities (individual users, teams, roles, or apps) that may bypass ruleset conditions on a protected branch; it must contain no automation identity the agent can act as (the agent, a bot PAT, a deploy key, or a CI app the agent uses), and is otherwise kept minimal and audited — a human maintainer may appear here in the solo profile as the documented escape hatch (small-team and org repos keep the list empty — a second human reviewer unblocks merges).
