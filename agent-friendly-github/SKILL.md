...
- Merge authority defaults to the human — an agent does not merge a PR it authored unless the human explicitly authorized the agent to merge it (a standing grant in `AGENTS.md` or an in-session instruction that specifically says the agent may merge); green required checks and a zero-review configuration are gate outcomes, not delegation.
