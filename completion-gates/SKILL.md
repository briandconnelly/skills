...
**Never make a gate disappear quietly. The only permitted escapes are `ABANDON: <id> <reason>` (the gate stays in the ledger as surrendered) and `amend --reason` (which may revise or remove gates — every change lands in the revision log).**
...
Freeze validates the file (unique ids, every criterion covered, every gate has an evidence slot) and records the specs in a manifest. From here on, changing a gate's CHECK/EXPECT/FOR is *spec drift*: the runner refuses to run until you record the change with `amend --reason "<why>"`, which appends a visible revision. Scope changes are allowed; silent ones are not.
...
- **When a gate's CHECK is new** (a new test, a bugfix probe), run `gate-check.mjs control <id>` after freezing and before implementing the fix, recording that the CHECK can fail — a check that cannot fail proves nothing. If the pre-fix state is already gone, say so in the report rather than skipping silently.
...
- Surface every manifest revision with its reason.
