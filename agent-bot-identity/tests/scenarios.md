...
> (1) the org-match grep is now left-anchored on the host (`(^|[@/])github\.com[:/]…`) so a spoofed host like `notgithub.com:acme/` can no longer yield a bot verdict;
