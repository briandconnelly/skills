...
> - Login shell is zsh. Claude Code is the agent; it runs shell commands locally as the user, and per-project environment variables can be injected into its shell sessions via `.claude/settings.local.json`.
...
- [ ] Local isolation via per-project env (`GIT_AUTHOR_*`/`GIT_COMMITTER_*`, `GIT_CONFIG_*` with credential-helper reset + bot helper, org-scoped `insteadOf` SSH→HTTPS rewrite, `commit.gpgsign false`).
- [ ] Any zsh-level PATH/shim activation goes in `~/.zshenv`, not `~/.zshrc` (non-interactive shells never source `.zshrc`), or the plan otherwise demonstrates awareness of the non-interactive-shell trap.
- [ ] Verification covers both directions: bot active inside an opted-in agent session AND personal terminal unchanged (helper, signing, SSH push).
...
**Expected baseline failures:** shim activation wired into `~/.zshrc` (never fires in non-interactive sessions), `checks: read`/`actions: read` omitted or misunderstood, isolation framed as a security boundary rather than attribution, no repo-side ruleset audit, `required_signatures` interaction unmentioned, token cache chmod race and hardcoded expiry.
...
- [ ] `~/.zshrc` guard flagged: non-interactive agent shells never source it, so the gh shim silently never activates; move to `~/.zshenv` and use `which gh` as the canary.
