...
| 2026-06-09 | 1 (design) | baseline | 1/9 | Envelope design put errors on stdout by design; no machine profile, no dry-run (used `--confirm`), no per-command error catalog, no content hash on export, no inspection/isolation affordances; timeout unit only in prose; passed only pagination (`has_next_page` + `page`). |
| 2026-06-09 | 1 (design) | with-skill | 9/9 | Declared machine profile with all flags in `global_flags`; `stderr_framing` field; `--dry-run` with `dry_run: true` and `dry_run_shape` on both mutations; `errors_possible` per command; `--timeout-ms` with `unit` field; export returns path/byte_size/content_hash; `whoami`/`config show --resolved`/`env` plus `--isolated`. |
