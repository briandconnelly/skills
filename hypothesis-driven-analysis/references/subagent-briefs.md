...
- Treat worker reports as claims, not conclusions; a deviation from brief downgrades the reconciled outcome to `NON_DISCRIMINATING` unless the deviation is verified harmless. "Verified harmless" means on evidence from outside the faulted return — a re-run, an independent source, or a figure the brief itself supplied. The rest of the return's own attestations cannot clear a fault they sit beside: internal consistency of the surviving fields is a plausible story, not a verification.
