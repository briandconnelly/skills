...
All datasets are CSVs under `tests/fixtures/s1-conversion/` (repo-root-relative; a dispatched prompt resolves it to an absolute path for the subagent):
...
- `deploys.log` — deploy events, one per line: `timestamp  service  version`.

## Recipes

```python
import pandas as pd
base = "tests/fixtures/s1-conversion"  # repo-root-relative; run from the repo root
