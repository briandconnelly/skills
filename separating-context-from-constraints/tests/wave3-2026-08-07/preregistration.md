...
- **E9c — report components present.** Recorded on **every wave-3 audit arm** — never on a control arm, never on scenario 15's author-usability agent — as three separate fields: two booleans and one three-valued field. Each is an **observation, never a scored endpoint, and never summed** with E1–E9 or with contract adherence.
  ...
  - `classification-table` (`true` / `false`): `true` when, apart from the findings, the output contains a table or list that assigns **two or more** statements of the target to Core Concept classes (binding rule, load-bearing fact, discretionary context); all three classes need not appear. A class label attached to a single statement, and class labels that appear only inside a finding's "why it fails" field, are `false`. Boundary examples: a table listing every statement with a class column is `true`; the sentence "'Deploys are cut weekly' is a load-bearing fact" standing alone is `false`; a finding whose "why it fails" opens "this is a binding rule, but…" is `false`.
  ...
  **E9a keeps its own scope.** E9a and its known-positive controls remain limited to scenarios 10 and 15 as stated above; E9c does not widen them. On those two scenarios, record E9a on the same line as E9c so the four unspecified report components sit together; on every other scenario, record E9c alone, and make no `absent` claim about the after-document there, because no control calibrates it outside scenarios 10 and 15.
