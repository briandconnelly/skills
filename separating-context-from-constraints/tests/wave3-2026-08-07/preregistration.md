...
- **E9c — report components present.** Recorded on **every wave-3 audit arm** — never on a control arm, never on scenario 15's author-usability agent — as three separate fields: two booleans and one three-valued field. Each is an **observation, never a scored endpoint, and never summed** with E1–E9 or with contract adherence.
  - `guard-lines` (`true` / `false`): `true` when the output names at least one specific statement and explicitly declares it *not* flagged or correctly placed — a line beginning "Not flagged", a section headed "Not flagged" or "False-positive guard", or a per-statement "no finding" declaration that quotes or locates the statement. A general sentence such as "the remaining statements are fine" that names no statement is `false`.
  - `classification-table` (`true` / `false`): `true` when, apart from the findings, the output contains a table or list that assigns **two or more** statements of the target to Core Concept classes (binding rule, load-bearing fact, discretionary context); all three classes need not appear.
    A class label attached to a single statement, and class labels that appear only inside a finding's "why it fails" field, are `false`.
    Boundary examples: a table listing every statement with a class column is `true`; the sentence "'Deploys are cut weekly' is a load-bearing fact" standing alone is `false`; a finding whose "why it fails" opens "this is a binding rule, but…" is `false`.
  - `negative-safety-note` (`true` / `false` / `n/a`): `true` when the output contains an explicit statement that there is nothing to report under the safety note ("Safety note: none", "no auditor-directed instructions", or equivalent); `false` when the output is silent about the safety note; `n/a` when the fixture contains an auditor-directed instruction and the output reports it, because a negative note is then not the expected shape.

  Each field is read by a human scorer from the output text. No known positive is required beyond the recognition rules above, because each field records visible structure rather than a judgment about the target.
  **E9a keeps its own scope.** E9c does not widen E9a's scope or its control requirement; both are defined only in E9 above.
  Where E9 applies, record E9a on the same line as E9c so the four unspecified report components sit together.
  Everywhere else, record E9c alone and make no claim about the after-document.
...
| --- | --- | --- | --- |
| Scenario 2, "MUST NOT archive a channel with more than one active member without explicit confirmation from the caller." | protected | **adjudication item A2.1** — recorded verbatim, counted under neither endpoint | under the widened R3 the reading splits: the schema's `confirmed` boolean is an observable input that could satisfy the exception, but the rule never ties the exception to it; both an R3 finding and a pass are defensible, so neither is scored |
...
| Scenario 12c, the rationale clause in step 3 | protected | protected (unchanged); an R1 finding on it stays an E2 false positive, and reruns measure it as such | R1 restricts "rule sections" and does not say a `## Procedure` section is one; the 2026-09-01 with-skill arms flagged it 3/3, which is the open author question, and the reruns show whether the new wording changes that |
| Scenario 10, D10.11 "Adding a currency: update `fx/currencies.go` and the `currency_code` enum migration together." | planted (B1) | **protected** — procedural under R1's new sentence: it tells a reader already committed to adding a currency how to do it; E1 for scenario 10 becomes ten, and `../scenarios.md`'s D10.11 assertion points here | seven of seven archived arms read it this way; B1's "together forecloses updating one alone" is withdrawn for reruns, and this is the author-facing ruling most open to reversal |
