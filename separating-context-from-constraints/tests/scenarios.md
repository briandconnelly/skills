...
| 2026-09-02 | 1 (rules buried in prose) | with skill ×3 (Wave B) | 5/5 ×3; E2 0 ×3 | `runs/2026-09-02-scenario1-with-skill-waveB.md`. Regression cell for the R1 embedding test: three buried rules under R1, material, with a rules section added, and no background sentence flagged. Reps 1 and 3 add a secondary R3 on the Test-plan commit scope; it sits on a planted statement, so it is an observation, not an E2 count. |
| 2026-09-02 | 2 (legitimate inline rules) | with skill ×3 (Wave B) | assertion 1 1/1 ×3; E2 strict 1 ×3 (per-statement 1 ×3) | `runs/2026-09-02-scenario2-with-skill-waveB.md`. The material R3 on the `MUST NOT` sentence recurs 3/3 and is recorded as adjudication item A2.1 under the preregistration's Rerun classification, counted under neither E1 nor E2; assertions 2 and 3 are the ones that exclusion affects and are not scored here. What remains is the rider: every rewrite adds a description to the schema's `confirmed` property, against the archived 2 per rep. |
| 2026-09-02 | 5 (clean document) | with skill ×3 (Wave B) | 4/4 ×3; E2 0 ×3 | `runs/2026-09-02-scenario5-with-skill-waveB.md`. Regression cell for R3's "ordinary domain predicate" clause: no rep raises R3 against rule 7's "multiple independent user-visible changes". |
| 2026-09-02 | 6 (unverifiable hedge and misplaced context) | with skill ×3 (Wave B) | 6/6 ×3; E2 0 ×3 | `runs/2026-09-02-scenario6-with-skill-waveB.md`. Regression cell for the R3 author-decision path: one consolidated R2 with R3 secondary, the migration sentence separately minor R1, and both count lines correct. All three reps were re-dispatches after HTTP 429. |
| 2026-09-02 | 8 (reachable conflict) | with skill ×3 (Wave B) | 4/4 ×3; R5 conflict 3/3; E2 0 ×3 | `runs/2026-09-02-scenario8-with-skill-waveB.md`. Regression cell for R5 with the `SKILL.md:59` field/object guard deleted; also the live known positive scenario 14 cites. |
| 2026-09-02 | 10 (long sectioned document) | with skill ×3 (Wave B) | E1 10/10 ×3; E2 0 ×3; standing 11/11 ×3 | `runs/2026-09-02-scenario10-with-skill-waveB.md`. Finding-for-finding identical to the archived cell; what changed is the denominator, since D10.11 is now protected rather than planted and is unflagged 3/3. E7 direction passes ×3 (D10.6, D10.7, D10.8, D10.9, D10.10 minor; D10.1–D10.5 material); E9a `absent` ×3. |
| 2026-09-02 | 12 (R1's compact/long criterion) | with skill ×3 (Wave B) | E2 strict 12a 0, 0, 0; 12b 2, 1, 3; 12c 2, 3, 2 | `runs/2026-09-02-scenario12-with-skill-waveB.md`. 12a: no section demand in any rep, so W9's 12a half closes. 12b: no R1 finding ×3; new R3/R5 findings on the seat-count and duplicate-call sentences (author question). 12c: every rep moves S3b into `## Rules` citing the new liftability clause, against 0/3 archived, and step 4 is unflagged ×3 — a direction reversal the registry still scores as a false positive, and the open author question. |
| 2026-09-02 | 13 (R3's scope) | with skill ×3 (Wave B) | E1 1/2, 2/2, 2/2; scored assertions 3/3 ×3; E2 0 ×3 | `runs/2026-09-02-scenario13-with-skill-waveB.md`. D13.2 is planted under R3 on rerun rather than recorded as an observation; two reps of three flag it and rep 1 applies the same wording's "ordinary domain predicate" clause to "major changes". D13.1 3/3, B13.1 unflagged ×3, E3 preserved ×3. |
| 2026-09-02 | 14 (R5 semantic scope, held out) | with skill ×3 (Wave B) | 4/4 ×3 clean; E2 0 ×3; known positive R5 3/3 | `runs/2026-09-02-scenario14-with-skill-waveB.md`. The `:59` guard is deleted in this blob, so this arm is both the with-skill cell and the ablation, and it reads clean ×3 as the 2026-09-01 cell did. The known positive is scenario 8's with-skill cell, cited by artifact path rather than rerun. The two cells were dispatched in different batches of the same rerun session, four minutes apart, against the same blob, model and harness; that deviates from N6's literal "dispatched together … in the same batch" wording while meeting its stated purpose, and is recorded for the author to rule on. |
| 2026-09-02 | 15 (ambiguity volume) | with skill ×3 (Wave B) | E1 10/10, 9/10, 10/10; E2 1 ×3; E3 preserved 27/27; standing 5/6 ×3 | `runs/2026-09-02-scenario15-with-skill-waveB.md`. The nine R2 defects are 9/9 ×3 with promoted and demoted rewrites and no silent selection. D15.6, planted on rerun, is flagged under R3 in reps 1 and 3 and missed in rep 2. D15.10 is flagged with an R1 secondary ×3, making it nine of nine across archived and rerun with-skill reps — an author question, not a variance. D15.8 is the adjudication item, excluded from both endpoints; E9a `absent` ×3. |
| 2026-09-02 | 18 (litmus question 1) | with skill ×3 (Wave B) | D18.1 3/3; assertions 4/4, 4/4, 3/4; E2 0, 0, 1 | `runs/2026-09-02-scenario18-with-skill-waveB.md`. Classification by function ×3 and D18.1 recalled under R1 ×3, with an R2 secondary and an author decision in reps 1 and 2. Rep 3 raises a minor R2 on Bg3 ("Feel free to skim…"), the cell's only E2 against 0 ×3 archived, and an open author question. |
| 2026-09-02 | Wave B rerun of the wave (`wave3-2026-08-07/results-waveB-2026-09-02.md`) | with skill ×3 per cell (Wave B) | strict E2 20 over the eight archived-comparable fixtures, against 23 archived | Eleven cells against `SKILL.md` blob `2cb876f4…` at commit `a9f0116`. Per-cell verdicts, the E2 comparison with per-fixture deltas, the W-item verdicts (W9, W10, W4/#160, W17, regression cells), the five author questions, and the process notes are in that file. |
