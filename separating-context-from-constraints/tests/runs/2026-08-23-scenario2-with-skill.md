...
Notes: the committed `SKILL.md` (blob 9cbb6ef0d086ddff0580cd17923ccf83ae1e1fa4) differs from the blob this arm read by one edit only — the Non-Goals grading bullet's parenthetical changed from "numeric scores are excluded by Summary Format" to "see Summary Format", a pointer-only change that owes no arm under the adopted protocol. Confirmation cell for the Non-Goals consolidation (cross-layer paraphrase removed); see "Why this cell" below.
